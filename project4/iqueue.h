...
class IQueue{
     ...
     ******************************************/
    Node * merge(Node *root, Node *newNode);//merges heaps
    void print(Node *root) const;//prints out crop info of each node
    int countCrops(Node *root) const;// counts crops an d returns a number
    Node * copy(Node * root);//copys the contents of heap to another
    bool minProperty(Node *root);//checks for min heap property
    bool maxProperty(Node *root);//checks for max heap property
};
#endif